  ...
  real(dp) :: total_energy,ekin_old,ekin_new,pm,mom_new
   ...
   do iy  = 1, nghost
      ...
      if(ix<=first_active .or. ix>=last_active) then
         ...
         u_prim(ibound_left,iP) =  u_prim(i_active_right,iP) +  half*(mom_new**2-u_prim(i_active_right,ivz)**2.)/u_prim(i_active_right,irho)

         mom_new  = u_prim(i_active_left,ivz) + pm* u_prim(i_active_left,irho)*q_shear*Omega_shear*box_l
         u_prim(ibound_right,ivz)        = mom_new
         u_prim(ibound_right,iP) = u_prim(i_active_left,iP) +  half*(mom_new**2-u_prim(i_active_left,ivz)**2.)/u_prim(i_active_left,irho)

#if NDUST>0         
         do idust=1,ndust
           u_prim(ibound_left,ivdz(idust))     = u_prim(i_active_right,ivdz(idust))  + pm*u_prim(i_active_right,irhod(idust))*q_shear*Omega_shear*box_l
           u_prim(ibound_right,ivdz(idust))    = u_prim(i_active_left,ivdz(idust)) + pm*u_prim(i_active_left,irhod(idust))*q_shear*Omega_shear*box_l
         end do
#endif  
      end if
#endif  
...
#if NDUST>0         
         do idust=1,ndust
            u_prim(ibound_left,ivdz(idust))     = u_prim(i_active_right,ivdz(idust)) - u_prim(i_active_right,irhod(idust))*q_shear*Omega_shear*box_l
            u_prim(ibound_right,ivdz(idust))    = u_prim(i_active_left,ivdz(idust))+ u_prim(i_active_left,irhod(idust))*q_shear*Omega_shear*box_l
         end do
#endif  
      end do
      !stop
