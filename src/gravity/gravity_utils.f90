...
subroutine mtot
  ...
  do i=first_active,last_active
     Mint(i)  = q(i,irho)*vol_cell(i)*fourpi
     M_tot    = M_tot+Mint(i)
     dmint(i) = q(i,irho)*dvol(i)*fourpi
#if NDUST>0     
     do idust=1,ndust
        Mint(i)    = Mint(i)+q(i,irhod(idust))*vol_cell(i)*fourpi
        M_tot_dust = M_tot_dust+q(i,irhod(idust))*vol_cell(i)*fourpi
...
subroutine read_gravity_params(ilun,nmlfile)
  ...
  integer :: io,ilun
  logical::nml_ok
  namelist/gravity_params/sink,l_soft,sink_acc_thre
   print *, "########################################################################################################################################"
   print *, "########################################################################################################################################"
   ...
   if(sink) print *, 'A sink is in the center,the accretion threshold is',sink_acc_thre
   if(l_soft.ne.0.0d0)print *, 'You are using a softening lenght of ',l_soft,' au'
