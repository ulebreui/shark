...
subroutine distribution_dust(initi)
  ...
  If(initi.and.(nrestart.eq.0)) then !Else we use the updated dust distribution
     if(trim(dust_distribution).eq.'mrn' .and. kernel_type==0) then
        ...
        do idust=1,ndust
           do i=1,ncells
              !epsilondust(i,idust)=((1.0d0+mminus(idust))*exp(-mminus(idust))-(1.0d0+mplus(idust))*exp(-mplus(idust)))
              epsilondust(i,idust)=mdust(i,idust)*exp(-mdust(i,idust))*(mplus(idust)-mminus(idust))
              !epsilondust(i,idust)=mdust(i,idust)*exp(-mdust(i,idust))
...
subroutine read_dust_params(ilun,nmlfile)
 ...
 & dtcontrol_growth
  ...
  if(coupled_dust) then
     print *, "We impose a strong coupling between gas and dust"
  endif
  ...
  if(kernel_type==2) then
     print*, 'Additive kernel'
     !eps_threshold=0!1d-100 
     !rhodust_threshold=0!1d-100 
     !dust_ratio_min=1d-300
  endif
