...
subroutine charge
  ...
  !$OMP PRIVATE(sigmas_ions,sigmas_dust,omegas_el,omegas_ions,omegas_dust,lowT)
  !$OMP DO
  do i=first_active,last_active
     ! Temperature
     T= barotrop(uold(i,irho)) 
     ! Grain size in cm
     ...
     ! Dust number density
     do idust=1,ndust
        n_k(idust)=uold(i,irhod(idust))*unit_d/(mdust(i,idust)*unit_m)
     end do
     ! Gas number density
     nH_loc=uold(i,irho)*unit_nh
     ...
     B_gauss = B_cell(i)  ! Magnetic field

     sigmav_dust(:)=pi*(l_grain_loc(:))**2.*dsqrt(8.0*kB*T/(pi*2.0d0*mH))*(1.0d0+dsqrt(pi/(2.*tau_k(:))))

     t_sdust(:)=(dsqrt(pi*gamma/8.0d0)*(rhograin)*(sdust(i,:)*unit_l)/(uold(i,irho)*unit_d*cs_eos(barotrop(uold(i,irho)))*unit_v))
